...
**Hash Format**:
```
$argon2id$v=19$m=19456,t=2,p=1$randomsalthere$hashoutputhere
│        │    │              │               │
│        │    │              │               └── The actual hash
│        │    │              └── Base64 salt
...
└─────────────────────────────────────────────────────────────────┘
        │                           ▲
        │ POST /login               │ { session_token, expires_at, user }
        │ { email, password }       │
        ▼                           │
┌─────────────────────────────────────────────────────────────────┐
│                      SERVICE LAYER                               │
...
│  1. validate_email()                                            │
│  2. get_user_by_email() ─────────────────────────┐             │
│  3. verify_password(input, stored_hash)          │             │
...
│ email: user@ex...   │                 │ user_id: uuid-123       │
│ password_hash: $... │                 │ token: "abc-def-..."    │
└─────────────────────┘                 │ expires_at: 2024-12-02  │
                                        └─────────────────────────┘
```
...
**Prefix Pattern**: `BUILDSCALE__` prevents conflicts with other apps.
...
- [ ] I understand the `BUILDSCALE__` prefix convention
...
> 2. Add environment variable with `BUILDSCALE__` prefix
