...
impl Tool for EditTool {
    ...
    async fn execute(
        ...
        let path = super::normalize_path(&edit_args.path);
        
        // Validation: old_string cannot be empty
        if edit_args.old_string.is_empty() {
        ...
        if !matches!(file.file_type, FileType::Document) {
             return Err(Error::Validation(ValidationErrors::Single {
                ...
                message: "Edit tool only supports Document file types".to_string(),
            }));
        }
        ...
        if let Some(last_read_hash) = edit_args.last_read_hash {
        ...
        // Search and Count
        let matches: Vec<_> = content_text.match_indices(&edit_args.old_string).collect();
        ...
        // Replace
        let new_content_text = content_text.replace(&edit_args.old_string, &edit_args.new_string);
